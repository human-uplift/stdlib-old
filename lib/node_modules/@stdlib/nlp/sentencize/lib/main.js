...
function isEndOfSentence( tokens, i ) {
        ...
        // Skip this token if the next token is another punctuation mark (e.g., "!!" or "!?")
        // Only the last punctuation mark in a sequence should be considered a sentence terminator
        if (
                ip1 < tokens.length && 
                RE_SENTENCE_ENDING.test(tokens[ ip1 ])
        ) {
                return false;
        }
