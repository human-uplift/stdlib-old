...
function isEndOfSentence( tokens, i ) {
	...
	// Check if the next token is a quotation mark, which indicates we're inside a quotation
	// and this punctuation mark should not be treated as an end-of-sentence marker
	if (
		(token === '.' || token === '!' || token === '?') &&
		ip1 < tokens.length && 
		RE_QUOTES.test(tokens[ip1])
	) {
		return false;
	}
